...
Analysis Distill is a snapshot-only model landscape tool. `solution.md` is authoritative for the implementation boundary and `distill.md` is the product contract.

A `distill run` invocation fetches the enabled current-state sources, validates and normalizes records in memory, resolves identities, computes deterministic current scores, renders a self-contained HTML report, and atomically publishes:
...
scheduler. `distill top` reads the root snapshot JSON. Missing source fields stay `null` or
...
```bash
python3 -m distill run --config config/app.yaml --output ./distil
python3 -m distill validate ./distil/snapshot.json
python3 -m distill top --output ./distil --plain
```
...
```bash
python3 -m distill run --config config/fixture.yaml --output ./out --source fixture

python3 -m distill top --output ./out --plain
```

To make output reproducible, pass an explicit timestamp:

```bash
python3 -m distill run --config config/fixture.yaml --source fixture --generated-at 2026-09-18T00:00:00+00:00
```

Useful commands include `distill validate`, `distill inspect`, and `distill sources`. Exit codes are `0` complete, `3` degraded optional source, `4` analysis/render/publication failure, `64` usage, `69` required source failure, `75` publication lock, and `78` configuration failure.
...
The report includes two organization Copilot distill tabs. Its catalog is configured in
...
- `.github/workflows/pages.yml` runs the live analysis daily at 02:17 UTC and supports manual dispatch. It validates the snapshot, creates `distil/index.html`, and deploys the complete `distil/` artifact to GitHub Pages.
...
The scheduled workflow does not create commits. A failed required source or validation step stops
deployment, leaving the previous Pages artifact available. GitHub schedules are approximate and may
be delayed; the manual dispatch is available for an immediate refresh.
